...
class OrderRepositoryDatabase(OrderRepository):
        ...
        async with asyncpg.create_pool(
            ...
            async with pool.acquire() as connection:
                await connection.execute(
                    'INSERT INTO ecommerce.order (id_order, cpf, code, total, freight)'
                    'VALUES ($1, $2, $3, $4, $5);',
                    order.id_order,
                    order.cpf,
                    order.code,
                ...
                for item in order.items:
                    await connection.execute(
                        'INSERT INTO ecommerce.item (id_order, id_product, price, quantity)'
                        'VALUES ($1, $2, $3, $4);',
                        order.id_order,
                        item.id_product,
                        item.price,
    ...
    async def get_by_id(self, id_order: str) -> OrderData:
        async with asyncpg.create_pool(
            database=db_name, host=db_host, port=db_port, user=db_user, password=db_password
        ) as pool:
            async with pool.acquire() as connection:
                row = await connection.fetchrow(
                    'SELECT * FROM ecommerce.order WHERE id_order = $1;', id_order
                )
                return OrderData(
                    id_order=row[0],
                    cpf=row[1],
